 ...
 *
 *	TODO:
 *		[-]		Shift UWB ranging code to it's own function
...
//   display.print("TAG --> "); display.println(uwbID, HEX);
  ...
  if (distance >= 1) {
	...
	// tone(BUZZ_PIN, middle_c, duration);
  }
//   display.println();
  display.setTextSize(1);
...
void alertBuzz() {
  if (distance >= 5.0) {
	...
	tone(BUZZ_PIN, alert_low, duration);
  } else if (distance <= 2.0 && distance > 1.0) {
	tone(BUZZ_PIN, alert_med, duration);
...
/** ==========[ NEW RANGE ]========== **
 *  Callback function for new range data
